...
architecture rtl of V1495_Demo2 is
  ...
  signal REG_R6     			: std_logic_vector(31 downto 0) := (others => 'Z');
	...
	signal user_output_C		: std_logic_vector(31 downto 0) := (others => 'Z');
	signal ndiv_length      : std_logic_vector(31 downto 0) := (others => 'Z');
  signal REG_STATUS		    : std_logic_vector(31 downto 0) := (others => '0');
	signal output_C			    : std_logic_vector(31 downto 0) := (others => 'Z');
  
 
  signal meb_clr     : std_logic := '0';
  signal meb_dti     : std_logic_vector(31 downto 0);
  signal meb_rd      : std_logic := '0';
  signal meb_wr      : std_logic := '0';
  signal meb_wrusedw : std_logic_vector(11 downto 0);
  signal meb_rdusedw : STD_LOGIC_VECTOR(11 DOWNTO 0);
  signal meb_dto     : std_logic_vector(31 downto 0);
  signal meb_rdempty : std_logic := '0';
  signal meb_wrfull  : std_logic := '0';

  signal pll_clk     : std_logic;
  signal areset      : std_logic;
  
  alias monin_green  : std_logic is  REG_STATUS(1);
  signal monout_green: std_logic;
  
  alias monin_red    : std_logic is  REG_STATUS(2);
  signal monout_red  : std_logic;
...
begin
  ...
  REG_R6(7 downto 4)   <= conv_std_logic_vector(2, 4);  -- Demo number
  ...
  instance_mb_fifo: mb_fifo
	...
	instance_pattern_rec: pattern_rec
		port map(
			clk     			=> pll_clk,
			A         		=> A,
			B         		=> B,
      ...
      nLBRES        => nLBRES,
      areset        => areset,
      meb_wrfull    => meb_wrfull,
      meb_wr        => meb_wr,
      meb_wrusedw   => meb_wrusedw,
			mask_A	      => mask_A,
			mask_B	      => mask_B,
			ctrlreg			  => ctrlreg,
			user_output_C	=> user_output_C,
      ndiv_length   => ndiv_length,
      REG_STATUS    => REG_STATUS 
		);
    
  instance_monost_green_led: monostable
    port map( 
      CLK        => pll_clk,
      RESET      => nLBRES,
      MONIN      => monin_green,
    ...
    instance_monost_red_led: monostable
    port map( 
      CLK        => pll_clk,
      RESET      => nLBRES,
      MONIN      => monin_red,
  ...
  instance_LB_INT: LB_INT 
      ...
      meb_dto     => meb_dto,
      meb_rd      => meb_rd,
      meb_rdempty => meb_rdempty,
      meb_rdusedw => meb_rdusedw,
